...
#define __WARN(...) {printf(KYEL); printf("[%d]: ", getpid()); printf(__VA_ARGS__); printf(KNRM);}
#define __ERR (...) {printf(KRED); printf("[%d]: ", getpid()); printf(__VA_ARGS__); printf(KNRM);}
#define __NTFY(...) {printf(KBLU); printf("[%d]: ", getpid()); printf(__VA_ARGS__); printf(KNRM);}

#define __CHECK(str) {if ((str) < 0) {printf(KRED); perror("Error_"); printf(KNRM); fflush(stderr); exit(EXIT_FAILURE);};}
