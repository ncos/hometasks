...
import socket, sys, thread, select, pickle, json, time
...
class Server:
    ...
    def send_matrix(self, sock, str_mat):
        try:
            sent = sock.send(str_mat + 'a')
            print "Sent %d bytes of the matrix from %d" % (sent - 1, len(str_mat))
        except:
            print "Send matrix fail"
            self.close_connection(sock)


    def compute(self, str_mat, matrix_size):
        server_result = []
        server_result.extend(xrange(matrix_size * (matrix_size + 1)))
        self.task_list_wid.extend(xrange(matrix_size * (matrix_size + 1)))
        ...
        while len(self.task_list_wid) + self.tasks_at_computation > 0:            
            ...
            for sock in read_sockets:
                if sock == self.server_socket: # Than we got a new connection
                    ...
                    self.send_matrix(sockfd, str_mat)
                else:
                    try:
                        data = self.get_a_terminated(sock)
                        print 'Server received %d bytes' % len(data)
                        print data
                        ...
                        if answer[0] == "MRQ": # New task is being requested
                            ...
                            #print task_id_list, ' <-> ', self.tasks_at_computation_map[str(sock.getpeername()[0])]
                            
                            print "Stat:"
                            print sock.send(json.dumps(task_id_list) + 'a')
                            print len(json.dumps(task_id_list) + 'a')
                            print 'Tasks remain: ' + str(len(self.task_list_wid))
                        else:
                            print "Receiving"
