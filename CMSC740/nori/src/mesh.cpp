...
void Mesh::activate() {
    ...
    if (this->m_emitter == NULL) return;

    for (uint32_t i = 0; i < this->getTriangleCount(); ++i)
        this->m_pdf.append(this->surfaceArea(i));
...
void Mesh::samplePosition(const Point2f& sample_, Point3f& p, Normal3f& n) const {
    ...
    // Normal
    if (this->m_N.size() > 0) {
        const Normal3f& n0 = this->m_N.col(i0), n1 = this->m_N.col(i1), n2 = this->m_N.col(i2);
        n = n0 * (1.0f - bary.x() - bary.y()) + n1 * bary.x() + n2 * bary.y();
        n.normalize();
        return;
    }

    n = Normal3f((p1 - p0).cross(p2 - p0).normalized());
}
