...
public:
    ...
    Color3f Li(const Scene *scene, Sampler *sampler, const Ray3f &ray) const {
        ...
        while ((int)light_path.size() < 10)
        {
            ...
            this->connect_eye( vert , (unsigned)light_path.size(), cam_origin, sampler);
        ...
        while (light_path_len <= (int)10)
        {
            ...
            // l = 1, e = 2..inf
            //if (light_path_len < 1)
            li += this->connect_light(vert, emitter, sampler) / pdf;

            // l = 2..inf, e = 2..inf
            for (unsigned j = 0; j < lps; ++j) {
                li += connect_path(light_path[j], vert);
            }
    ...
    Color3f connect_path( const BDPT_Vertex& p0 , const BDPT_Vertex& p1) const {
    ...
    Color3f connect_light(const BDPT_Vertex& eye_vertex, const Mesh *light, Sampler *sampler) const {
	    ...
	    float emissionPdfW = light->getBSDF()->pdf(bsdf_record);
        ...
        * eye_vertex.rr;
        ...
        float weight = 1.0f / ( mis0 + mis1 + 1.0f );
	    return li * weight;
    }


    void connect_eye(const BDPT_Vertex& light_vertex, int len, Point3f &cam_origin, Sampler *sampler) const {
        ...
        Color3f radiance = light_vertex.throughput * bsdf_value * gterm / sample_per_pixel;
      
        const float lightvert_pdfA = cosAtLightVertex * invSqrLen;
